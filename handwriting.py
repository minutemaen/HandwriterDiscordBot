...
#                     help='amount to slant lines, 0 means no slant, positive means upwards, negative means downwards',
#                     metavar='VAL')
...
def apply(images, a, timestamp: int, cropped: bool = False):
    print(images)
    image_paths = []
    for path in images:
    ...
    for img in image_paths:
        if img not in n_image_paths:
            n_image_paths.append(img)
    print(n_image_paths)
    num_images = len(n_image_paths)
    print(f'Will process {num_images} images.')
    background_paths, background_codes = get_background_codes(len(n_image_paths), "background/")
    files = []
    for i in range(num_images):
        ...
        try:
            ...
            edited = do_artifact(img, back,
                                 ...
                                 line_slant_factor=1,
                                 line_move_factor=1,
                                 text_fade_factor=0.5
                                 )

            save_path, ext = os.path.splitext(os.path.basename(path))
            if not os.path.exists(f"out/{a}/"):
                os.mkdir(f"out/{a}/")
            if not os.path.exists(f"out/{a}/{timestamp}/"):
                os.mkdir(f"out/{a}/{timestamp}/")
            if cropped:
                file = cv2.imwrite(
                    f'out/{a}/{timestamp}/{i}-cropped.png',
                    edited
                )
            else:
                file = cv2.imwrite(
                    f'out/{a}/{timestamp}/{i}.png',
                    edited
                )
